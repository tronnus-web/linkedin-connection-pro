...
function startSingleTabAutomation() {
  ...
  const recoveryTimeout = setTimeout(() => {
    ...
    if (AppState.isRunning) {
      ...
      // Move to next profile - don't close tab as it might still be loading
      moveToNextProfile();
    }
  ...
  if (!AppState.automationTabId) {
    chrome.tabs.create({ 
      ...
      if (chrome.runtime.lastError) {
        ...
        // Move to next profile immediately for invalid URLs or other tab creation errors
        moveToNextProfile();
        return;
      }
      ...
      setTimeout(function() {
        ...
        if (!AppState.isRunning) {
          console.log("Automation stopped during page load wait");
          return;
        }
        
        // Check if the tab still exists before trying to send a message
        chrome.tabs.get(tab.id, function(tabInfo) {
          if (chrome.runtime.lastError) {
            console.log("Tab was closed before we could send message:", chrome.runtime.lastError.message);
            ...
            if (AppState.isRunning) {
              ...
              // Clear recovery timeout as we're handling this error now
              if (AppState.currentRecoveryTimeout) {
                clearTimeout(AppState.currentRecoveryTimeout);
                AppState.currentRecoveryTimeout = null;
              }
    ...
    chrome.tabs.update(AppState.automationTabId, { 
      ...
      if (chrome.runtime.lastError) {
        ...
        if (AppState.isRunning) {
          ...
          // Update failed - tab may be closed or invalid URL
          AppState.automationTabId = null;
          saveState();
      ...
      setTimeout(function() {
        ...
        // Check if the tab still exists
        chrome.tabs.get(AppState.automationTabId, function(tabInfo) {
          if (chrome.runtime.lastError) {
            console.log("Tab was closed before we could send message:", chrome.runtime.lastError.message);
...
function moveToNextProfile() {
  ...
  // Make sure any existing recovery timeout is cleared
  if (AppState.currentRecoveryTimeout) {
    clearTimeout(AppState.currentRecoveryTimeout);
    AppState.currentRecoveryTimeout = null;
  }
...
function exportAnalyticsToCSV() {
  ...
  dates.forEach(date => {
    ...
    // Add row to CSV (without acceptance data)
