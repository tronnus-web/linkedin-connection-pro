...
function recordConnection(success, failureReason = null) {
  ...
  // Send result to background script with template information
  if (success) {
    // Add template info to profile data
    if (extractedProfileData && analyticsData.templateUsed) {
      extractedProfileData.templateUsed = analyticsData.templateUsed;
    }
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  if (message.action === 'sendConnection' && !isProcessing) {
    ...
    // Check if the page loaded properly first
    if (!checkPageValidity()) {
      console.log("Page validation failed, aborting connection attempt");
      isProcessing = false;
      return true;
    }
